...
public class GatheringSend {
	...
	public ArrayList<?> getAuditInfo(){
		ArrayList<ReceiptsPO> poList;
		ArrayList<ReceiptsVO> voList=new ArrayList<ReceiptsVO>();
		try {
			poList = sendData.findRAudit();
			for(Iterator<ReceiptsPO> i=poList.iterator();i.hasNext();){
	...
	public ResultMessage EndAudit(ArrayList<?> list){
		for(Iterator<ReceiptsVO> i=(Iterator<ReceiptsVO>) list.iterator();i.hasNext();){
				try{
					ReceiptsVO vo=i.next();
					sendData.updateReceiptsPO(new ReceiptsPO(vo.getDate(), vo.getFee(),
				vo.getCourier(), vo.getOrdernumbers(), vo.getOffice(),
				vo.getdCondition()));
				}catch(Exception e){
					e.printStackTrace();
