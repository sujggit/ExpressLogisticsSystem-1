...
public class AManagementPanel extends JPanel{
	...
	public static FreeReportPage addStaffPanel,fixStaffPanel,seekStaffPanel;
	...
	private static void createSeekAgencyPanel(){
		...
		BaseUI.myAdd(bpanel,submit,constraints,0,1,2,1);
		submit.addMouseListener(new MouseAdapter(){
			@Override
			public void mouseClicked(MouseEvent arg0){
				ResultMessage resultMessage=ResultMessage.SUCCESS;
				...
				if(agencyVO!=null){
					...
					idNumber.setText("机构编号：  ");
					JLabel name=new JLabel("机构名:  "+agencyVO.getName());
					JLabel phoneNumber=new JLabel("电话号码:  "+agencyVO.getPhoneNumber());
					JLabel address=new JLabel("地址:  "+agencyVO.getAddress());
					JLabel leader=new JLabel("负责人:  "+agencyVO.getLeader());
					JLabel idJLabel = new JLabel(agencyVO.getIDNumber());
					JButton seekStaff = new JButton("查看人员");


					BaseUI.myAdd(bpanel, idJLabel, constraints, 1, 0, 1, 1);
					BaseUI.myAdd(bpanel,name,constraints,0,1,1,1);
					BaseUI.myAdd(bpanel,phoneNumber,constraints,0,2,1,1);
					BaseUI.myAdd(bpanel,address,constraints,0,3,1,1);
					BaseUI.myAdd(bpanel,leader,constraints,0,4,1,1);
					BaseUI.myAdd(bpanel, seekStaff, constraints, 0, 5, 1, 1);
					BaseUI.myAdd(bpanel,sure,constraints,0,6,1,1); 


					ArrayList<String>staff = agencyVO.getStaff();
					seekStaff.addMouseListener(new MouseAdapter() {
						public void mouseClicked(MouseEvent arg0){
							...
							try{
								...
								createSeekStaffPage(staff);
								tab.addTab(title, TransportUI.createPage(seekStaffPanel));
								FreePagePane pagePane = FreeUtil.getPagePane(seekStaffPanel);
								tab.setSelectedComponent(pagePane);
							}
	...
	public static void createSeekStaffPage(ArrayList<String> arrayList){
		DefaultTableModel model = new DefaultTableModel();
		model.addColumn("工作编号");
		model.addColumn("姓名");
		model.addColumn("身份证号码");
		model.addColumn("职位");
		model.addColumn("联系方式");


		for (String workNumber:arrayList) {
			try {
				StaffVO vo=pManagementService.select(workNumber);
				Vector row = new Vector();
				row.add(vo.getIdNumber());
				row.add(vo.getName());
				row.add(vo.getIdNumber());
				row.add(vo.getWork());
		...
		FreeToolbarButton finish=createButton("/free/test/print.png", "结束查看", true);
	...
	public  static void refresh(){
		...
		ResultMessage resultMessage = ResultMessage.SUCCESS;
		ArrayList<AgencyVO>agencyVOs = null;
