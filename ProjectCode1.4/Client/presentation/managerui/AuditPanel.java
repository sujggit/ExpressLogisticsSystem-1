...
public class AuditPanel {
	...
	private static ArrayList<IoputVO> Ioput;
	...
	public  static void refresh(){
		...
		delivers = auditService.getDeliver();
		Receipts = auditService.getReceipts();
		Ioput = auditService.getIoput();
		...
		table.addMouseListener(new MouseListener() {
			...
			public void mouseReleased(MouseEvent arg0) {
				...
				if(selectRows==1&&columnIndex!=0){
					...
					}else if(type.equals(enums.Ioput.in)||type.equals(enums.Ioput.out)){
						String title = type;
						IoputVO vo = Ioput.get(selectRow - orderNum - transNum - paymentNum - receiptsNum - deliverNum);
						JPanel panel = createIoputFixPanel(vo, selectRow - orderNum - transNum - paymentNum - receiptsNum -deliverNum);
						tab.addTab(title, panel);
						FreePagePane pagePane = FreeUtil.getPagePane(panel);
						tab.setSelectedComponent(pagePane);
				...
				for(int i=0;i<model.getRowCount();i++){
					...
					if((boolean) model.getValueAt(i, 0)){
						if(i<orderNum){
							...
							DeliverVO vo = delivers.get(i-orderNum-transNum-paymentNum);
							vo.setDocumentCondition(DocumentCondition.audited);
							delivers.set(i-orderNum-transNum-paymentNum, vo);
						}else if(i<(orderNum+transNum+paymentNum+ioputNum+receiptsNum+deliverNum)){
							IoputVO vo = Ioput.get(i-orderNum-transNum-paymentNum-receiptsNum-deliverNum);
							vo.setdCondition(DocumentCondition.audited);;
							Ioput.set(i-orderNum-transNum-paymentNum-receiptsNum-deliverNum, vo);
	...
	protected static JPanel createIoputFixPanel(IoputVO vo, int i) {
		// TODO Auto-generated method stu
		JPanel fixIoputPanel = new JPanel();
		...
		JLabel ioput = new JLabel("单据型号：  " + vo.getIoput());
		JLabel id = new JLabel("快递单号：  "+ vo.getID());
		JLabel condition = new JLabel("货物状态：  " + vo.getCondition());
		JLabel time = new JLabel("处理时间：  " + vo.getTime());
		JLabel destination = new JLabel("目的地：  " + vo.getDestination());
		JLabel position = new JLabel("出发地：  " + vo.getPosition());
		JLabel writer = new JLabel("拟写人：  " + vo.getNameOfWriter());

		BaseUI.myAdd(bpanel, ioput, constraints, 0, 0, 1, 1 );
		BaseUI.myAdd(bpanel, id, constraints, 0, 1,  1, 1);
		BaseUI.myAdd(bpanel, condition, constraints, 0, 2, 1, 1);
		BaseUI.myAdd(bpanel, time, constraints, 0, 3, 1, 1);
		BaseUI.myAdd(bpanel, destination, constraints, 0, 4, 1, 1);
		BaseUI.myAdd(bpanel, position, constraints, 0, 5, 1, 1);
		BaseUI.myAdd(bpanel, writer, constraints, 0, 6, 1, 1);
		...
		submit.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				tab.remove(fixIoputPanel);
			}
		});

		fixIoputPanel.add(bpanel);

		return fixIoputPanel;
	...
	protected static JPanel createDeliverFixPanel(DeliverVO vo, int selectRow) {
		...
		JPanel fixDeliverPanel = new JPanel();
		...
		JLabel deliverID = new JLabel("派件单号：    " + vo.getID());

		JButton submit = new JButton("确认");
		BaseUI.myAdd(bpanel, submit, constraints, 0, 3, 2, 1);
		submit.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				tab.remove(fixDeliverPanel);
			}
		});



		JLabel date = new JLabel("派送日期:  "+vo.getTime());
		JLabel officer = new JLabel("营业厅业务员:  "+ vo.getWriter());	
	...
	protected static JPanel createTransportFixPanel(TransportVO transportVO, int selectRow) {
		...
		id.setText("中转|装运单编号:  ");
		JLabel idNumber = new JLabel(transportVO.getID());
		...
		MaskFormatter maskTranfficNumber = null;
		try {
		...
		BaseUI.myAdd(bpanel, idNumber, constraints,1, 1, 1, 1);
	...
	public static FreeReportPage createFixOrderPage(ArrayList<String>order,ArrayList<Condition>condition){
		...
		MaskFormatter maskOrderNumber = null;
		try {
